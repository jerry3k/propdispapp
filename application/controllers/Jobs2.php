...
class Jobs2 extends MY_Controller {
	...
	public function index() {
		...
		$this->load->view( 'jobs2', $data );
	}

	public function save() {
		$this->load->model( 'Jobs2_model' );
		$xss_clean = $this->security->xss_clean( $this->input->post() );
		echo $this->Jobs2_model->save( $xss_clean );
		exit();
	}
	...
	public function load_data() {
		...
		$rows = $this->Jobs2_model->get_jobs( $xss_clean );
	...
	public function delete() {
		...
		$this->load->model( 'Jobs2_model' );
		$xss_clean = $this->security->xss_clean( $this->input->post() );
		echo $this->Jobs2_model->delete( $xss_clean );
		exit();
	}
	...
	public function status() {
		...
		$xss_clean = $this->security->xss_clean( $this->input->post() );
		echo $this->Jobs2_model->status( $xss_clean );
		exit();
	}
	...
	public function client_locations() {
		...
		$this->load->model( 'Jobs2_model' );
		$rows  = $this->Jobs2_model->get_client_location( 'id=' . $id );
		$array = [];
		if ( ! empty( $rows ) ) {
	...
	public function get_jobtype_price() {
		...
		$this->load->model( 'Jobs2_model' );
		$row = $this->Jobs2_model->get_jobtype_price( $xss_clean );
		header( 'Content-Type: application/json' );
	...
	public function monthly_report() {
		...
		$this->load->model( 'Jobs2_model' );
		$rows = $this->Jobs2_model->get_monthly_report( $xss_clean );
		header( 'Content-Type: application/json' );
		echo json_encode( $rows );
		exit();
	}
	...
	public function load_sticker() {
		if ( empty( $_POST ) ) {
			echo 'Job id missing.';
			exit();
		}

		$result = $this->db->where( 'job_id', $_POST['id'] )->get( 'jobsattr' )->result_array();
		if ( empty( $result ) ) {
			echo 'empty';
			exit();
		}
	...
	public function del_sticker_row() {
		if ( empty( $_POST ) ) {
			echo 'Job id missing.';
			exit();
		}

		$this->db->delete( 'jobsattr', [ 'id' => $_POST['id'] ] );
		echo 'OK';
		exit();
	}

	public function sticker_status() {
		if ( empty( $_POST ) ) {
			echo 'Job id missing.';
			exit();
		}

		$this->db->update( 'jobsattr', [ 'status' => $_POST['status'] ], [ 'id' => $_POST['id'] ] );
